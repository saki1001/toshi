...
function sendAndRetrieve(theForm, theDiv, loadingStr) {
	...
	var cObj = YAHOO.util.Connect.asyncRequest('POST', 'inner.php', {success: success, failure: success});
	return false;
}
	...
	return {
		...
		get_content: function(e) {
	   		if(SUGAR.unifiedSearchAdvanced.usa_content == null) { 
				...
				var cObj = YAHOO.util.Connect.asyncRequest('GET','inner.php?to_pdf=1&module=Home&action=UnifiedSearch&usa_form=true', 
		...
		getAdditionalDetails: function(bean, id, spanId) {
			go = function() {
				...
				if(typeof SUGAR.util.additionalDetailsCalls[spanId] != 'undefined') // call already in progress
					return;
				ajaxStatus.showStatus(SUGAR.language.get('app_strings', 'LBL_LOADING'));
				url = 'inner.php?to_pdf=1&module=Home&action=AdditionalDetailsRetrieve&bean=' + bean + '&id=' + id;
...
SUGAR.savedViews = function() {
	...
	return {
		...
		shortcut_select: function(selectBox, module) {
			document.location.href = 'inner.php?module=SavedSearch&search_module=' + module + '&action=index&saved_search_select=' + selectBox.options[selectBox.selectedIndex].value;
		},
		handleForm: function() {
	...
	return {
		...
		searchFormSelect: function(view, previousView) {
			...
			if(document.getElementById(module + theView + 'SearchForm').innerHTML == '') { 
				...
				url = 	'inner.php?module=' + module + '&action=ListView&search_form_only=true&to_pdf=true&search_form_view=' + theView;
		...
		clear_form: function(form) {
			var newLoc = 'inner.php?action=' + form.action.value + '&module=' + form.module.value + '&query=true&clear_query=true';
