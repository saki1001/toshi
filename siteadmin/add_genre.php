...
if($_REQUEST['del']!="")
{
	...
	$sql=mysql_query("UPDATE genre SET $qrr where id='".trim($_GET['id'])."'");	
	header("location:add_genre.php?id=".$_REQUEST['id']);
	exit;
}
...
if(isset($_POST['Submit']))
{
	if($_GET['id'])
	{
		$strQueryPerPage="select * from genre where name='".addslashes($_POST["name"])."' and  id!=".$_GET['id'];
		...
		if($strTotalPerPage<1)
		{
			...
			header("location:manage_genre.php?msgs=3");
		}
		else 
		{
	...
	{
		$strQueryPerPage="select * from genre where name='".addslashes($_POST["name"])."'";
		$strResultPerPage=mysql_query($strQueryPerPage);
		$strTotalPerPage=mysql_affected_rows(); 
		if($strTotalPerPage<1)
		{
			...
			header("location:manage_genre.php?msgs=1");
			exit;
		}
		else 
		{
...
if($_GET['id'])
{
	...
	$SEL="SELECT * from genre where id='".$_GET['id']."'";
        ...
        <tbody >
          <tr>
            ...
            <td width="80%" valign="top" align="center"><table width="100%"  border=0 cellpadding="2" cellspacing="2">
                <tr>
                  <td height="35" class="form111"><? if($_GET['id']){?>Edit<? } else {?>Add<? } ?> Genre</td>
                </tr>
                <tr>
                  <td height="222" class="formbg" valign="top">
                      ...
                      <table cellspacing="2" cellpadding="2" width=98% border="0" class="t-b">
                        ...
                        <tr>
                          <td width="21%" height="25" align="right" valign="top"><strong><span class="a">*</span> Genre:&nbsp;</strong></td>
