...
//////////////  DISPLAY THE NUMERIC PAGING WITHOUT RECORD DETAIL///////////////////CREATED BY PARAS PITRODA
	...
	function number_pageing_front($query,$record_per_page='',$pages='')
	{
			return $this->number_pageing_front1($query,$record_per_page,$pages,"","Y");
	}
	function number_pageing_debbies_simple($query,$record_per_page='',$pages='')
	{
			return $this->number_pageing_debbies_simple_1($query,$record_per_page,$pages,"","Y");
	}
...
////////////
//////////////////By ////////////////
function number_pageing_front1($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='')
{
		...
		$this->tbl="<table width='100%' height='100%' border='0' cellpadding='0' cellspacing='0' ><tr><td width='15%' align='left'>";
		
		
		if($start>0)
		{ 
			$this->tbl.="<a href='".$this->file_names."?start=".($start-$this->record_per_page).$tmpva."'  class='link2' >Previous</a>&nbsp;&nbsp;"; 
		} 

		$this->tbl.="</td><td width='70%' class='blogDate'  align='center'>&nbsp;";
		...
		if($simple!='N')
		{
			for($i=$start_loop;$i<$end_loop;$i++) 
			{
				if($current_page==$i)	
				{
					$this->tbl.="<span class='link2'><strong>".$i."</strong></span>&nbsp;&nbsp;";	
				}	
				else 
				{ 
					$this->tbl.="<a href='".$this->file_names."?start=".($i-1)*$this->record_per_page.$tmpva."'  class='link2' >".$i."</a>&nbsp;&nbsp;"; 
				}
			}
		}
		...
		if($start+$this->record_per_page<$totalrows) 
		{ 
			$this->tbl.="<a href='".$this->file_names."?start=".($start+$this->record_per_page).$tmpva."' class='link2'>Next</a>"; 
		} 
		$this->tbl.="</td></tr></table>";
...
function number_pageing_debbies_simple_1($query,$record_per_page='',$pages='',$detail='',$bottom='',$simple='')
{
		...
		$Mcat=$_GET['mcat'];
		if($totalrows>$record_per_page)
		{
					$this->tbl="<table width='250' align='right' border='0' cellpadding='0' cellspacing='0' ><tr><td width='15%' align='left'>";
					
					
					if($start>0)
					{ 
						$this->tbl.="<a class='paging2_off' href='".$this->file_names."?start=".($start-$this->record_per_page).$tmpva."' >Previous</a>&nbsp;&nbsp;"; 
					} 
					...
					if($simple!='N')
					{
						for($i=$start_loop;$i<$end_loop;$i++) 
						{
							if($current_page==$i)	
							{
								$this->tbl.="<span class='paging2_on'>".$i."</span>&nbsp;&nbsp;";	
							}	
							else 
							{ 
								$this->tbl.="<a class='paging2_off' href='".$this->file_names."?start=".($i-1)*$this->record_per_page.$tmpva."'  >".$i."</a>&nbsp;&nbsp;"; 
							}
						}
					}
					...
					if($start+$this->record_per_page<$totalrows) 
					{ 
						$this->tbl.="<a class='paging2_off' href='".$this->file_names."?start=".($start+$this->record_per_page).$tmpva."' >Next</a>"; 
					} 
					$this->tbl.="&nbsp;</td></tr></table>";
		}
		if($bottom=="Y")
...
//////////////  END OF ORDER FUNCTION///////////////////CREATED BY PARAS PITRODA
